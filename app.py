...
def load_admin_password():
    """Read the admin password from admin_password.txt.

    Environment fallback is kept only for backward compatibility with older
    Render configurations. The file takes precedence so the password can be
    changed directly from GitHub as requested.
    """
    try:
        value = ADMIN_PASSWORD_FILE.read_text(encoding="utf-8").strip()
    except OSError:
        value = ""
    if value:
        return value
...
def admin_login():
    ...
    if request.method == "POST":
        ...
        if not admin_password:
            error = "رمز مدیر تنظیم نشده است. مقدار فایل admin_password.txt را وارد کن."
